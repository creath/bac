...
uint256 hashGenesisBlock("0x0000024d7816ebf6a994893cb0377e05cba08ee64d3bc9b059ab0f24b5229fe0");
...
const string strMessageMagic = "BitAlphaCoin Signed Message:\n";
...
{
    // BitAlphaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // BitAlphaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        // Genesis block
        const char* pszTimestamp = "22-9-2015 Genesis Block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0.1 * 10 * 10000000;
	    ...
	    assert(block.hashMerkleRoot == uint256("0x8e36f5fb55de1af5e1f40786fa7f3bb3dbe9cc47dc2ba764171a7f2134a4681f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xee, 0xcc, 0xad, 0xe2 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitAlphaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitAlphaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitAlphaCoinMiner(CWallet *pwallet)
{
    printf("BitAlphaCoinMiner not supported!\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BitAlphaCoin-miner");
    ...
    {
        printf("BitAlphaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitAlphaCoinMiner, pwallet));
}
